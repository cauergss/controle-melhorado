...
export default function EstoquePage() {
  ...
  async function deleteProduct() {
    if (!editingProduct) return;
    if (!confirm(`Tem certeza que deseja deletar o produto "${editingProduct.name}"? Esta ação não pode ser desfeita.`)) {
      return;
    }
    setDeleting(true);
    try {
        ...
        <div className="modal-overlay" onClick={closeEditModal}>
          <div className="modal-panel p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={saveEdit} disabled={saving || deleting} className="btn btn-primary flex-1">
                {saving ? <span className="spinner" /> : null}
                {saving ? "Salvando..." : "Salvar"}
              </button>
              <button onClick={deleteProduct} disabled={deleting || saving} className="btn btn-secondary flex-1" style={{ color: "var(--danger)", borderColor: "var(--danger-border)", background: "var(--danger-bg)" }}>
                {deleting ? <span className="spinner" /> : (
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ display: "inline", marginRight: "0.25rem" }}>
                    <polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/>
                  </svg>
                )}
                {deleting ? "Deletando..." : "Deletar"}
              </button>
              <button onClick={closeEditModal} disabled={saving || deleting} className="btn btn-secondary flex-1">Cancelar</button>
            </div>
          </div>
        </div>
      )}
